...
/*********************** 上传项配置区 开始 ***********************/
//获取域名
$http = (_isHttps() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'];
...
if(isset($_POST['iceEditor-img']) && $_POST['iceEditor-img']){
	$fileExt = _fileExt($_POST['iceEditor-img']);
...
foreach ($keys as $key){
	...
	//获取文件的名称
	$fileName = _filePre($name);

	//获取文件的后缀名称
	$fileExt = _fileExt($name);
	...
	if(!$error){
			...
			if(in_array($fileExt, $allowed_image_types)) {
				compressImage(URL.$dir.$name, URL.$dir.$name, 85);
			}
		}
}
