	...
	class iceEditor {
		constructor(id) {
			...
			this.drag.onmousedown = function (e) {
				...
				var y = e.clientY;
				// 使用 parseInt 解析当前高度，处理 "150px" 格式
				var currentHeight = parseInt(_z.iframe.height) || 150;
				window.onmousemove = function (e) {
					var h = e.clientY - y;
					var newHeight = currentHeight + h;
					if (newHeight >= 100) {
						_z.iframe.height = newHeight + 'px';
						_z.height = newHeight + 'px';
					} else {
			...
			this.init(); //初始化参数
			callback && callback.call(this,this); //先执行回调，让用户可以设置参数
			this.create(); //创建编辑器
			this.paste(); //监听粘贴
		}
		id(a) {
		...
		repositionPopup(popup) {
			var popupMain = popup.querySelector('.iceEditor-popupMain');
			var windowWidth = window.innerWidth;
			var windowHeight = window.innerHeight;
			var popupWidth = popupMain.offsetWidth;
			var popupHeight = popupMain.offsetHeight;
		...
		createMenu(json) {
			...
			if (json.dropdown) {
				...
				function showMenu() {
					if (li.hideTimer) {
						clearTimeout(li.hideTimer);
						li.hideTimer = null;
					}
					if (li.openMenu) {
						div.className = 'iceEditor-menuDropdown iceEditor-menuActive';
					}
				}
				
				// 隐藏下拉菜单（带延迟）
				function hideMenu() {
					li.hideTimer = setTimeout(function() {
						div.className = 'iceEditor-menuDropdown';
					}, 300);
				}
				
				// 菜单项鼠标事件
				li.onmouseenter = showMenu;
					...
					menu[i].onclick = function () {
						...
						switch (this.attr) {
							...
							case 'code':
								...
								//最大化
							case 'max':
								var webHeight = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
								var webWidth = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
								
								// 保存当前高度（在最大化之前）
								if (!_z.originalHeight) {
									_z.originalHeight = _z.iframe.height;
								}
								
								// 保存原始样式
								if (!_z.originalStyle) {
									_z.originalStyle = {
										position: _z.editor.style.position,
										zIndex: _z.editor.style.zIndex,
										width: _z.editor.style.width,
										height: _z.editor.style.height,
										top: _z.editor.style.top,
										left: _z.editor.style.left,
										borderRadius: _z.editor.style.borderRadius,
										border: _z.editor.style.border
									};
								}
								
								_z.editor.style.position = 'fixed';
								_z.editor.style.zIndex = '99999';
								_z.editor.style.width = '100vw';
								_z.editor.style.height = '100vh';
								_z.editor.style.top = '0';
								_z.editor.style.left = '0';
								_z.editor.style.borderRadius = '0';
								_z.editor.style.border = 'none';
								
								// 计算工具栏高度
								var toolHeight = _z.tool.offsetHeight || 40;
								// 设置iframe高度为视口高度减去工具栏高度和拖拽条高度
								_z.iframe.height = (webHeight - toolHeight - 12) + 'px';
								_z.iframe.width = '100%';
							...
							case 'min':
								// 恢复原始样式
								if (_z.originalStyle) {
									_z.editor.style.position = _z.originalStyle.position;
									_z.editor.style.zIndex = _z.originalStyle.zIndex;
									_z.editor.style.width = _z.originalStyle.width;
									_z.editor.style.height = _z.originalStyle.height;
									_z.editor.style.top = _z.originalStyle.top;
									_z.editor.style.left = _z.originalStyle.left;
									_z.editor.style.borderRadius = _z.originalStyle.borderRadius;
									_z.editor.style.border = _z.originalStyle.border;
								} else {
									_z.editor.removeAttribute('style');
								}
								// 恢复原始高度配置
								_z.iframe.height = _z.originalHeight || _z.height;
								_z.iframe.width = '100%';
								this.parentNode.style.display = 'none';
							...
							default:
								...
								// 特别处理居中对齐 - 确保图片也能正确居中
								if (b[0] === 'justifyCenter' || b[0] === 'justifyLeft' || b[0] === 'justifyRight') {
									var selection = _z.w.getSelection();
									if (selection.rangeCount > 0) {
										...
										// 如果找到图片，使用包裹div的方式确保居中
										if (img) {
											var wrapper = _z.w.document.createElement('div');
											if (b[0] === 'justifyCenter') {
												wrapper.style.textAlign = 'center';
											} else if (b[0] === 'justifyLeft') {
												wrapper.style.textAlign = 'left';
											} else if (b[0] === 'justifyRight') {
